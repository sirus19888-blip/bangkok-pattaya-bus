...
export function StationCard({
  ...
  return (
    <section className="rounded-2xl border border-[#eadcc7] bg-[#fffaf2] p-3 shadow-sm sm:p-5 lg:p-3.5">
      ...
      <div className="mt-3 grid gap-3 sm:mt-5 sm:grid-cols-2 lg:mt-3 lg:grid-cols-1 lg:gap-3 xl:grid-cols-1">
        {stations.map((station, index) => (
          <article
            key={station.id}
            className="overflow-hidden rounded-2xl border border-[#eadcc7] bg-white shadow-sm"
          >
            <div className="border-b border-[#eadcc7] bg-[#f9fbff] p-3.5 sm:p-4 lg:p-3">
              <div className="flex items-start gap-3">
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#13233a] text-sm font-black text-white lg:h-7 lg:w-7 lg:text-xs">
                  {index + 1}
                </span>
                <div>
                  <h2 className="text-lg font-black leading-tight text-[#13233a] sm:text-xl lg:text-base">
                    {station.name}
                  </h2>
              ...
              <p className="mt-3 rounded-xl border border-[#eadcc7] bg-white px-3 py-2 text-sm font-semibold leading-6 text-[#4f5d6c] lg:mt-2 lg:text-xs lg:leading-5">
                <span className="font-black text-[#13233a]">{labels.tip}</span>{" "}
                {station.tip}
              </p>
            </div>
            <div className="space-y-3 p-3 sm:p-4 lg:space-y-2.5 lg:p-3">
              <StationPhotoGallery
