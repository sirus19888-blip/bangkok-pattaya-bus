...
const slug = "ekkamai-bus-terminal-to-pattaya-guide";
const path = `/en/${slug}`;
const canonicalUrl = `https://www.bangkokpattayabus.com${path}`;
const expectedSubId = "bpb-bangkok-to-pattaya-guide_ekkamai";
...
assert(
  existsSync(htmlPath),
  "Guide must be prerendered as a 200 HTML page. Run npm run build first.",
);
...
assert.equal(h1Matches.length, 1, "Ekkamai guide must render exactly one H1.");
assert.match(
  html,
...
assert.match(html, /href="\/"/, "Guide must link internally to the homepage.");
assert.match(
  html,
  /Is Ekkamai Bus Terminal the best station for Pattaya\?/,
  "Guide FAQ question must be present in HTML.",
);
assert.match(
  html,
  /Ekkamai is usually the easiest choice if you are staying near Sukhumvit or BTS Ekkamai\./,
  "Guide FAQ answer must be present in HTML.",
);

const guideCta = findAnchorByText(html, "Check Bangkok → Pattaya tickets");
...
const clickTest = simulateAffiliateClickForTest({
  ...
  from: "bangkok",
  href: `https://12go.asia/en/travel/bangkok/pattaya?z=15791301&sub_id=${expectedSubId}`,
...
console.log("Ekkamai guide checks passed.");
